...
The build produces `../media-manifest.json` from the committed local copies. It
is deterministic and records each file's SHA-256 digest, size, MIME type and a
digest-query URL so the PWA can identify unchanged files across deployments.
